...
def write_cli_catalog(repo_root: Path, apis: list[APIDef], cli_names: dict[str, dict[str, str]]) -> None:
    ...
    verbs = cli_names.get("verbs", {})
    resources = cli_names.get("resources", {})
    lines = [
    ...
    for api in apis:
        command = commands.get(api.operation_id, kebab_case(api.operation_id))
        lines.extend(
            [
                "\t{",
                f'\t\tGroup:       "{groups.get(api.group_code, kebab_case(api.group_code))}",',
                f'\t\tCommand:     "{command}",',
                f'\t\tVerb:        "{verbs.get(api.operation_id, default_cli_verb(api))}",',
                f'\t\tResource:    "{resources.get(api.operation_id, command)}",',
                f'\t\tAPIID:       "{api.api_id}",',
...
def default_cli_verb(api: APIDef) -> str:
    if api.response_content_type != "application/json":
        return "download"
    if api.operation_id == "list":
        return "search"
    if api.operation_id == "corpCode":
        return "list"
    return "get"
